<%@include file="header.jsp"%>
  ...
  <body>
    ...
    <div class="col s8 main">
      <sql:update>
        UPDATE notifications SET status='1' WHERE ${contactNumber}=notificationTo
      </sql:update>
      ...
      <c:choose>
        <c:when test="${userInfo.rows[0].type == 'farmer'}">
          <sql:query var="notifications">
            SELECT notifications.itemId, items.name, users.fname, users.contactNumber, notifications.date, cart.ID, cart.quantity FROM
            notifications,items,users, cart WHERE notifications.notificationTo=${contactNumber} and notifications.itemId=items.id
            and users.contactNumber=notifications.notificationFrom and notifications.cartId = cart.ID and available=1;
          </sql:query>
            ...
            <div class="col s12 m4 offset-m1">
              <div class="card blue-grey darken-1">
                ...
                <div class="card-action white-text">
                  ...
                  <a href="handleNotification.jsp?status=0&cartId=${notification.id}">Decline</a>
                </div>
              </div>
            </div>
            <c:if test="${loop.index % 2  == 1}">
        ...
        <c:otherwise>
          <sql:query var="items">
            SELECT id, Name, Description, itemId, notificationFrom, notificationTo FROM items,notifications where id=itemId and ${contactNumber}=
            notificationTo and available=1
          </sql:query>
          ...
          <div class="fixed-action-btn">
            ...
            <ul>
              <li>
                <a href="cart.jsp" class="btn-floating green tooltipped" data-position="left" data-delay="50" data-tooltip="Add to Cart"><i class="material-icons">shopping_basket</i></a>
              </li>
              <li>
